...
<div class="space-y-2">
    ...
    <p class="text-sm text-slate-500">
        Buat akun baru untuk administrator, mentor, maupun peserta pelatihan.
    </p>
</div>
...
<!-- Kartu Formulir Tambah Pengguna -->
<div class="bg-white border border-slate-200 rounded-xl p-8 sm:p-10 shadow-sm max-w-2xl">
    <form action="<?= base_url('admin/users/store') ?>" method="POST" class="space-y-6">
        ...
        <div>
            ...
            >
                <option value="peserta_b2c" <?= old('role', 'peserta_b2c') === 'peserta_b2c' ? 'selected' : '' ?>>Peserta B2C (Individu/Mandiri)</option>
                <option value="mentor" <?= old('role') === 'mentor' ? 'selected' : '' ?>>Mentor (Instruktur Praktisi)</option>
                <option value="peserta_b2b" <?= old('role') === 'peserta_b2b' ? 'selected' : '' ?>>Peserta B2B (Mitra Institusi)</option>
                <option value="super_admin" <?= old('role') === 'super_admin' ? 'selected' : '' ?>>Super Admin (Pengelola Sistem)</option>
            </select>
        </div>
        ...
        <div>
            ...
            <p class="text-[11px] text-slate-400 mt-1">
                Password akan di-hash secara aman menggunakan algoritma standar industri.
            </p>
        </div>
